...
class EditoraController {
    ...
    static cadastrarEditora = (req, res) => {
        ...
        editora.save((err) => {
            if(err){
                res.status(400).send({message: `${err.message} - Falha ao cadastrar Editora`});
            } else {
    ...
    static excluirEditora = (req, res) => {
        ...
        editoras.findByIdAndDelete(
            id,             
            (err) => {
                if(!err){
                    res.status(204).send({message: "Editora excluída com sucesso"})
                } else {
                    res.status(400).send({message: err.message})
                }
        })
